...
import com.ygs.imgproc.util.ImageUtils;
import com.ygs.imgproc.util.colorspace.RGB;

import java.awt.image.BufferedImage;

public class FreichenEdgeDetector extends AbstractEdgeDetector {

    private static final double FREICHEN_THRESHOLD_RATIO = 2.0; //Suggested range .2 - .4
    private static final int FREICHEN_STD_DEV = 3;
    public FreichenEdgeDetector(BufferedImage bufferedImage) {
        super(bufferedImage);
    }

    @Override
    public void draw() {
        BufferedImage edges = freichenEdges(FREICHEN_STD_DEV, FREICHEN_THRESHOLD_RATIO);
        edged = new RGB[edges.getWidth()][edges.getHeight()];
        for(int x=0;x<edges.getWidth();x++){
            for(int y=0;y<edges.getHeight();y++){
                edged[x][y] = new RGB(edges.getRGB(x,y));
            }
        }
     ...
     */
    public  BufferedImage freichenEdges(int numberDeviations, double fract) {
        int[][] raw = null;
        int[][] blurred = null;
        ...
        if (img != null && numberDeviations > 0 && fract > 0) {
            ...
            gx = Freichen.Horizontal(blurred);  //Convolved with 3x3 horizontal Sobel mask
            gy = Freichen.Vertical(blurred);    //Convolved with 3x3 vertical Sobel mask
